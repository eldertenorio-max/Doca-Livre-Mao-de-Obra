...
export function IntroSplash({ loading = false, onFinish }: Props) {
  ...
  useEffect(() => {
    const tick = window.setInterval(() => {
      setProgress((prev) => {
        ...
        if (!loading && minDone) return Math.min(100, prev + 6)
        if (loading) return Math.min(88, prev + 1.2 + Math.random() * 2.5)
